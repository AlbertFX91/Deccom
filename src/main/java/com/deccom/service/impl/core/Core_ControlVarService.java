...
public class Core_ControlVarService {
    ...
    public void executeMonitorize(Core_ControlVar controlVar) {
    	...
    	if(hasExtractor(connection)) {
    		...
    		dataExtractor = injectConnectionInExtractor(dataExtractor, connection);
	...
	private Core_DataExtractor injectConnectionInExtractor(Core_DataExtractor extractor, Core_Connection connection) {
		Map<String, Core_Connection> fields = new HashMap<>();
		fields.put("dataConnection", connection);
		propertiesInjection(extractor, fields);
		return extractor;
	}
	private Core_DataExtractor getExtractorByConnection(Core_Connection connection) {
		Object wrapper;
